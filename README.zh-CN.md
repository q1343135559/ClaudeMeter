...
**某一段突然消失了。** 本地缓存里已经**跨过重置时刻**的窗口，旧百分比是*可证伪的错误*而不只是旧，
绝不会被显示出来。`5H` / `WEEK` 有 stdin 实时兜底，所以直接丢弃；按模型窗口（如 `FABLE`）没有
任何其它数据来源，丢弃会让它无声消失——所以它改为显示 `?` 占位（如 `FABLE ░░░░░░ ? ·21h`），
表示"窗口已重置、新读数还没拿到"。执行一次 `/usage` 刷新缓存后就会恢复成正常百分比。
